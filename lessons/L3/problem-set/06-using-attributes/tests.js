...
function performSubmission() {
  ...
  // check if title has been added to all headers
  grader.addTest(function() {
    return grader.hasCorrectClass($('h1'), 'title') && grader.hasCorrectClass($('div h2:nth-of-type(1)'), 'title') && grader.hasCorrectClass($('div h2:nth-of-type(2)'), 'title') && grader.hasCorrectClass($('div h2:nth-of-type(3)'), 'title');
  }, {
    wrongMessage: "The class \"title\" should be applied to all headers. Have you added \"title\" to every header?"
  });

  // check if to-do-list has been added to the main div
  grader.addTest(function() {
    return grader.hasCorrectId($('div'), 'to-do-list');
  }, {
    wrongMessage: "The id \"to-do-list\" needs to be added to the HTML. What element would make the most sense to have the id \"to-do-list\"?"
  });

  // check if headers for each to-do-list are underlined
  grader.addTest(function() {
    return grader.hasCorrectClass($('div h2:nth-of-type(1)'), 'underline') && grader.hasCorrectClass($('div h2:nth-of-type(2)'), 'underline') && grader.hasCorrectClass($('div h2:nth-of-type(3)'), 'underline');
  }, {
    wrongMessage: "The headers for each to-do-list need to be underlined. Remember, you can add multiple classes to elements."
  });

  // check if brainstorm task has been crossed out
  grader.addTest(function() {
    return grader.hasCorrectClass($('ul:eq(1) li:nth-child(1)'), 'finished');
  }, {
    wrongMessage: "The task \"brainstorm ideas for Science project\" should be marked out. Try using the class \"finished\" on the list element."
  });

  // check if cal 2 task has been crossed out
  grader.addTest(function() {
    return grader.hasCorrectClass($('ul:eq(1) li:nth-child(2)'), 'finished');
  }, {
    wrongMessage: "The task \"finish Calculus 2 problems\" should be crossed out. The \"finished\" class could come in handy here."
  });

  // check if invitations task has been crossed out
  grader.addTest(function() {
    return grader.hasCorrectClass($('ul:eq(2) li:nth-child(1)'), 'finished');
  }, {
    wrongMessage: "The task \"send out invitations\" is not crossed out? Make sure you add the correct class to the list element in order to mark out the task."
  });

  // check if unordered lists have the class list
  grader.addTest(function() {
    return grader.hasCorrectClass($('ul:eq(0)'), 'list') && grader.hasCorrectClass($('ul:eq(1)'), 'list') && grader.hasCorrectClass($('ul:eq(2)'), 'list');
  }, {
    wrongMessage: "Did you remove a class? Each list should have the class \"list\"."
  });

  // check if sweep garage task has been crossed out
  grader.addTest(function() {
    return grader.hasCorrectClass($('ul:eq(0) li:nth-child(4)'), 'finished');
  }, {
    wrongMessage: "Did you remove a class? The task \"sweep the garage\" is not crossed out?"
  });

  // check if biology task has been crossed out
  grader.addTest(function() {
    return grader.hasCorrectClass($('ul:eq(1) li:nth-child(5)'), 'finished');
  }, {
    wrongMessage: "Did you remove a class? The task \"find sources for Biology research paper\" is not crossed out?"
  });
	...
	result = {
	  ...
	  congrats: "Nice job! Using attributes, like id and class, make styling webpages super quick!"
	};
	return result;
};